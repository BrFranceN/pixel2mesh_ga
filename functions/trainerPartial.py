...
class TrainerPartial(CheckpointRunner):
    ...
    def train(self):
        ...
        for epoch in tqdm(range(self.epoch_count, self.options.train.num_epochs),desc="Training Progress"):
            self.epoch_count += 1
            ...
            for step, batch in tqdm(enumerate(train_data_loader), total=len(train_data_loader), desc="Batch Progress"):
                # Send input to GPU
