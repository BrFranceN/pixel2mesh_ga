...
# KNN
from multivectorloss import knn_gather, knn_points
...
class PGALoss(nn.Module):
    ...
    def forward(self, source_points: torch.Tensor, target_points: torch.Tensor, device='cuda'):
        ...
        # print("source_points type: ", type(source_points))
        # exit()
...
if __name__ == "__main__":
    ...
    # Example tensors
    source_points_1 = torch.randn((batch_size, num_points, 3),requires_grad=True).to('cuda')  # Input partial point cloud
    ...
    # Compute loss
    loss = loss_fn(source_points_1 , target_points)
